...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="btn-group">
            <a href="{{ route('jurusan.inbox.index') }}" class="btn btn-secondary">
                <i class="bi bi-arrow-left"></i> Kembali ke Inbox
            </a>
            <form method="POST" action="{{ route('jurusan.inbox.notify', $proposal->id) }}" class="ms-2">
                @csrf
        ...
        <div class="card-body">
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <div><strong>Dosen KJFD:</strong> {{ $proposal->dosenKjfd?->name ?? '-' }}</div>
                </div>
            </div>
            <div class="mb-3">
                <strong>Judul Proposal:</strong>
                <div class="border rounded p-3 bg-light">{{ $proposal->judul }}</div>
            </div>

            <div class="d-flex gap-2">
                @if($proposal->file_path)
                    <a href="{{ route('jurusan.proposals.view-file', $proposal->id) }}" class="btn btn-primary" target="_blank">
                ...
                @endphp
                <a href="{{ route('jurusan.proposals.index', $bidangCode) }}" class="btn btn-outline-secondary">
